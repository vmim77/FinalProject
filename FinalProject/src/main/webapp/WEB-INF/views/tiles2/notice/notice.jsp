...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("input#searchWord").keyup(function(){
			...
			if(wordLength == 0) {
				...
				$.ajax({
					url:"<%= request.getContextPath()%>/noticeWordSearchShow.univ",
					type:"GET",
	...
	<%--location.href="<%= ctxPath%>/view.action?seq="+seq;--%>
		...
		frm.method = "GET";
		frm.action = "<%= request.getContextPath()%>/noticeView.univ";
		frm.submit();
	...
	function goSearch() {
		...
		frm.method = "GET";
		frm.action = "<%= request.getContextPath()%>/notice.univ";
	...
	<div class="pt-5 pb-5">	
		<form name="searchFrm" style="margin-top: 20px; margin-bottom: 15px;">
			...
			<c:if test="${sessionScope.loginuser.authority == 1}">
				<button type="button"  onclick="javascript:location.href='<%= request.getContextPath()%>/noticeAdd.univ'" class="btn btn-secondary float-right" style="font-size: 10pt; padding: 5px 12px; text-align: right;">+ 글쓰기</button>&nbsp;
			</c:if>
		</form>
