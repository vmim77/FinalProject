...
public class FileManager {
	...
	// === 파일 업로드 하기 첫번째 방법 === 
	   ...
	   // 리턴값 : 서버에 저장된 새로운 파일명(예: 2021110809271535243254235235234.png)
	   public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
	      ...
	      // pathname 은 예를 들면, C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\resources\files\2021110809271535243254235235234.png 이다.   
	   ...
	   public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
	       ...
	       // 운영체제가 UNIX 또는 Linux 이라면 File.separator 은 "/" 이다.
	       // 해당경로에 \ 를 더하고 파일명을 더한 경로까지 나타내어준 파일명(문자열)을 만든다. 
	       // pathname 은 예를 들면, C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\resources\files\20211108144610590594605730900.jpg 이다.
		   try {
		   ...
		   try {
			   ...
			   if(file.exists()) { // 실제로 다운로드할 해당 파일이 존재한다라면 
				   ...
				   byte[] readByte = new byte[4096]; // 파일을 읽어들일때 4킬로바이트만큼 읽어들일 것이다.
				   // 다운로드할 파일의 내용을 읽어오는 단위크기를 4096 byte로 하는 byte 배열 readByte 를 생성한다. 
				   
				   
				   BufferedInputStream bfin = new BufferedInputStream(new FileInputStream(file));  //FileInputStream는 파일을 읽어온다. 파라미터에 파일객체가 나온다. 파일이 존재하지 않을때의 익셉션처리 해야 함
				   ...
				   while( (length = bfin.read(readByte, 0, 4096)) != -1) { // 0부터 4096만큼 읽어들이고 그걸 readByte에 저장함
				   /*
		   ...
		   return false; //올바를때만 true값이 오고 잘못되면 리턴값에 false가 들어가도록 할 것이다.
	...
	// === 파일 삭제하기 === //
	   public void doFileDelete(String saveFilename, String path) throws Exception {
	      ...
	      String pathname = path + File.separator + saveFilename;
	    //서비스에서 파일명과 경로 파라미터에 값 넣음. 파라미터로 받아온 두가지값 사용해 변수에 넣음
	      File file = new File(pathname);
	...
	public String doFileUpload(InputStream is, String originalFilename, String path) throws Exception {
		...
		String newFilename = null;
	      ...
	      fos.flush(); //왼쪽은 안적어도 괜찮음. 내부적으로 오토플러쉬가 되기때문이다.
